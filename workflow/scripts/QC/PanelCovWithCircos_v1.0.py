...
def LocalCircosDraw(File4ChrBed, SPNames, Files4Cov, File4Plot, MaxCount, WidthSize, GenType):
	...
	# Circos Plot
	# Initialize Circos from BED chromosomes (each chr a line, from 0 to Int)
	ThisDir = os.path.dirname(os.path.abspath(__file__))
	circos = Circos.initialize_from_bed(File4ChrBed, space = 2, start = 1, end = 359, endspace = False)
	...
	# Map draw
	#Col4Track = ["tomato", "olive", "blue", "magenta", "lime", "grey", "skyblue", "tomato"]
	Col4Track = ["#FF6347", "#CD9B1D", "#4169E1", "#9370DB", "#A2CD5A", "#9C9C9C"]
	for sector in circos.sectors:
		# Plot chr color track
		...
		for i in range(len(SPNames)):
			...
			Data4y = [int(float(x)) for x in CovInfo[SPNames[i]][sector.name.replace("chr", "")]['y']]
			...
			track.fill_between(Data4x, Data4y, color = Col4Track[i], vmin = 0,vmax = float(MaxCount[i]))
			# Plot track labels
			#if sector.name == circos.sectors[0].name:
			#	circos.text(SPNames[i], r = track.r_center, deg = 0, color = Col4Track[i], size=11)
...
def main():
 ...
 This script was used to draw depth distribution of Panel data.
 
'''
	parser = MyParser(description=Descrip,formatter_class=argparse.RawDescriptionHelpFormatter)
	parser.add_argument('-bed',metavar='', help='( Required ) Bed format file seperated by each chr', required=True, dest='File4ChrBed', action='store')
	...
	LocalCircosDraw(args.File4ChrBed, args.SPNames, args.Files4Cov, args.CircosPlot, args.MaxCount, args.WidthSize, args.GenType)
